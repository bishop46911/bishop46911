...
class Trader:
    ...
    def __init__(self) -> None:
        ...
        self.spread_max = 41.78
        self.mean_spread_coco = 0
        self.std_spread_coco = 13.38
        self.coco_price_const = -4393.5047
        self.coco_price_coeff = 0.5029

        self.price_coconut_mean = 9999.9009
        self.price_coupon_mean = 635.0464

        self.std_spread_coeff = 1.5
    ...
    def coco_spread_strategy(self, state : TradingState, n_round : int):
        """
        ...
        self.update_ema_prices(product=COCONUT, state=state, alpha=self.coco_alpha)
        self.update_ema_prices(product=COCONUT_COUPON, state=state, alpha=self.coco_alpha)

        #historical self.mean_spread_coco=0
        #fixed regression for now, later test updating regression
        #lr_new = LinearModel()
        #lr_new.fit(self.ema_prices[COCONUT], self.ema_prices[COCONUT_COUPON])
        #self.coco_price_coeff, self.coco_price_const = lr_new.coef_, lr_new.intercept_


        fair_price_coupon = self.coco_price_coeff * mid_price_coconut + self.coco_price_const + self.mean_spread_coco
        spread = mid_price_coupon - fair_price_coupon
        logger.print(f'spread:{spread}')

        self.spread_max = max(abs(self.spread_max), abs(spread))
        spread_ratio = -1 * spread / self.spread_max

        target_pos_coupon = np.round(spread_ratio * self.position_limit[COCONUT_COUPON])

        self.mean_spread_coco = (self.mean_spread_coco * (HISTORICAL_ROUNDS + n_round - 1) + spread - self.mean_spread_coco) / (HISTORICAL_ROUNDS + n_round)
        
        self.std_spread_coco = np.sqrt((self.std_spread_coco**2 * (HISTORICAL_ROUNDS + n_round - 2) + (spread-self.mean_spread_coco)**2) / (HISTORICAL_ROUNDS + n_round - 1))
        
        #mean_spread_coco and std_spread_coco have not changed much in live data
        logger.print(f'mean_spread_coco: {self.mean_spread_coco}')
        logger.print(f'std_spread_coco: {self.std_spread_coco}')
        
        target_pos_coconut = -1 * np.round(spread_ratio * (self.price_coconut_mean/self.price_coupon_mean) * \
                                        self.position_limit[COCONUT])
        
        self.price_coconut_mean = (self.price_coconut_mean * (HISTORICAL_ROUNDS + n_round - 1) + mid_price_coconut) / (HISTORICAL_ROUNDS + n_round)
        self.price_coupon_mean = (self.price_coupon_mean * (HISTORICAL_ROUNDS + n_round - 1) + mid_price_coupon) / (HISTORICAL_ROUNDS + n_round)

        cur_pos_coconut, cur_pos_coupon = self.get_position(COCONUT, state), self.get_position(COCONUT_COUPON, state)
        
        if ((cur_pos_coupon==self.position_limit[COCONUT_COUPON] and target_pos_coupon>0) \
                    or (cur_pos_coupon==-1*self.position_limit[COCONUT_COUPON] and target_pos_coupon<0)):
            return [], []
        
        # short coupon and long coconut, market order only for now
        if spread >= self.std_spread_coeff * self.std_spread_coco:
            logger.print(f'target_pos coconut, coupon: {target_pos_coconut, target_pos_coupon}')
            logger.print(f'cur_pos coconut, coupon: {cur_pos_coconut, cur_pos_coupon}')
            ...
            coupon_price_adjustment=self.calculate_lambda_adjusted_r3(
                                    price_distance=spread
                                    , required_pos=target_pos_coupon
                                    , L_p=2, k_p=0.05
                                    , L_d=1, k_d=0.05)
            
            coupon_sell_orders_fair_price = np.round(coupon_best_bid - coupon_price_adjustment)
            #cap sell price at the mid price
            coupon_sell_orders_fair_price = min(coupon_sell_orders_fair_price, mid_price_coupon)

            #do partial fill if we will reach the position limit, only happen if cur_pos_dict[GIFT_BASKET] < 0
            # if -50+-20<-60, target_pos = -60--50 = -10
            if cur_pos_coupon + target_pos_coupon < -1 * self.position_limit[COCONUT_COUPON]:
                new_target_coupon = -1 * self.position_limit[COCONUT_COUPON] - cur_pos_coupon
                #target_pos_chocolate/target_pos_gb = -1 * (self.price_chocolate_mean/self.price_gift_basket_mean) * self.position_limit[CHOCOLATE]/self.position_limit[GIFT_BASKET])
                
                new_target_coconut = new_target_coupon * (-1 * (self.price_coconut_mean/self.price_coupon_mean) * self.position_limit[COCONUT]/self.position_limit[COCONUT_COUPON])
                
                #if any of components has reached the limit after this trade, don't trade at this round
                if (cur_pos_coconut+new_target_coconut > self.position_limit[COCONUT]):
                    new_target_coconut = self.position_limit[COCONUT] - cur_pos_coconut
                    new_target_coupon = new_target_coconut * (-1 * (self.price_coupon_mean/self.price_coconut_mean) * self.position_limit[COCONUT_COUPON]/self.position_limit[COCONUT])
                
                    if new_target_coconut==0 or new_target_coupon==0:
                        return [], []

                target_pos_coupon, target_pos_coconut = new_target_coupon, new_target_coconut
                logger.print(f'new target_pos coconut, coupon: {target_pos_coconut, target_pos_coupon}')
            #coco_orders_map_dict[COCONUT_COUPON].append(Order(COCONUT_COUPON, math.floor(coupon_sell_orders_fair_price), int(target_pos_coupon)))
            coco_orders_map_dict[COCONUT_COUPON].append(Order(COCONUT_COUPON, coupon_best_bid, int(target_pos_coupon)))
            ...
            coconut_price_adjustment=self.calculate_lambda_adjusted_r3(
                                price_distance=spread
                                , required_pos=target_pos_coconut
                                , L_p=2, k_p=0.05
                                , L_d=1, k_d=0.05)
            coconut_buy_orders_fair_price = np.round(coconut_best_ask + coconut_price_adjustment)
            #cap buy price at the mid price
            coconut_buy_orders_fair_price = max(coconut_buy_orders_fair_price, self.get_mid_price(COCONUT, state))
            #logger.print(f'product:{product}')
            #logger.print(f'buy_orders_fair_price:{buy_orders_fair_price}')
            
            #coco_orders_map_dict[COCONUT].append(Order(COCONUT, math.ceil(coconut_buy_orders_fair_price), int(target_pos_coconut)))
            coco_orders_map_dict[COCONUT].append(Order(COCONUT, coconut_best_ask, int(target_pos_coconut)))

            return coco_orders_map_dict[COCONUT_COUPON], coco_orders_map_dict[COCONUT]
        
        # long coupon and short coconut
        elif spread <= -1 * self.std_spread_coeff * self.std_spread_coco:
            logger.print(f'target_pos coconut, coupon: {target_pos_coconut, target_pos_coupon}')
            logger.print(f'cur_pos coconut, coupon: {cur_pos_coconut, cur_pos_coupon}')
            ...
            coupon_price_adjustment=self.calculate_lambda_adjusted_r3(
                                    ...
                                    , L_p=2, k_p=0.05
                                    , L_d=1, k_d=0.05)
            coupon_buy_orders_fair_price = np.round(coupon_best_ask + coupon_price_adjustment)
            #cap buy price at the mid price
            coupon_buy_orders_fair_price = max(coupon_buy_orders_fair_price, mid_price_coupon)

            if cur_pos_coupon + target_pos_coupon > self.position_limit[COCONUT_COUPON]:
                new_target_coupon = self.position_limit[COCONUT_COUPON] - cur_pos_coupon

                new_target_coconut = new_target_coupon * (-1 * (self.price_coconut_mean/self.price_coupon_mean) * self.position_limit[COCONUT]/self.position_limit[COCONUT_COUPON])
                
                #if any of components has reached the limit after this trade, don't trade at this round
                if (cur_pos_coconut+new_target_coconut < -1 * self.position_limit[COCONUT]):
                    new_target_coconut = -1 * self.position_limit[COCONUT] - cur_pos_coconut
                    new_target_coupon = new_target_coconut * (-1 * (self.price_coupon_mean/self.price_coconut_mean) * self.position_limit[COCONUT_COUPON]/self.position_limit[COCONUT])
            ...
            coco_orders_map_dict[COCONUT_COUPON].append(Order(COCONUT_COUPON, coupon_best_ask, int(target_pos_coupon)))
            ...
            coconut_price_adjustment=self.calculate_lambda_adjusted_r3(
                                    ...
                                    , L_p=2, k_p=0.05
                                    , L_d=1, k_d=0.05)
            coconut_sell_orders_fair_price = np.round(coconut_best_bid - coconut_price_adjustment)
            #cap sell price at the mid price
            coconut_sell_orders_fair_price = min(coconut_sell_orders_fair_price, self.get_mid_price(COCONUT, state))
            
            coco_orders_map_dict[COCONUT].append(Order(COCONUT, coconut_best_bid, int(target_pos_coconut)))

            return coco_orders_map_dict[COCONUT_COUPON], coco_orders_map_dict[COCONUT]
                
        #if edge minus mean is within -+1 * self.std_spread_coeff * self.std_spread_coco, don't trade
        else:
            return [], []
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        coupon_orders, coconut_orders = self.coco_spread_strategy(state, n_round=self.round)
